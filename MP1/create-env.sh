...
aws elb create-load-balancer --load-balancer-name krp-elb --listeners "Protocol=HTTP,LoadBalancerPort=80,InstanceProtocol=HTTP,InstancePort=80" --availability-zones us-west-2b --security-groups $3
...
aws autoscaling create-auto-scaling-group --auto-scaling-group-name itmo-544-as --launch-configuration-name $5 --availability-zones us-west-2b --min-size 0 --max-size 5 --desired-capacity 1
